...
export const ticketApi = {
  async getTickets(filters: TicketFilters = {}): Promise<Ticket[]> {
    ...
    const resp = await wsClient.send<ViewTicketsBackendResponse>(req);
  ...
  async favorite(token: string, ticketId: number, action: 'add' | 'remove') {
    return wsClient.send({ type: 21, token, index: String(ticketId), action });
  },
};
