...
namespace logsys
{
    enum class TimeZoneMode { Local, UTC };
    ...
    public:
        ...
        // 转换为字符串 格式: 秒[.微秒]
        std::string toString(TimeZoneMode mode = TimeZoneMode::UTC) const;
        // 转换为格式化字符串 showMic 是否显示微秒部分 格式: YYYY/MM/DD HH:MM:SS[.微秒]
        std::string toFormattedString(bool showMic = true, TimeZoneMode mode = TimeZoneMode::Local) const;
        // 转换为适合文件名的字符串 格式: YYYYMMDD-HHMMSS[.微秒]
        std::string toFileString(bool showMic = true, TimeZoneMode mode = TimeZoneMode::UTC) const;
